...
def gen_key_from_excel(excel_path, nrof_sheet=3, selected_col_names=['基因', 'Chrom:Pos Ref/Alt']):
    ...
    assert len(sheet_names) == nrof_sheet, 'Only found %2d sheets in excel: ' %(len(sheet_names), excel_path)
    ...
    print(excel_path)
    if len(keys_set) > 0:
        print('\n'.join([','.join(d) for d in list(keys_set)[:10]]))
    print('Finishing generating %8d key...'%len(keys_set))
    return keys_set
...
def process_pairs(pairs):
    key_intersect = set()
    for path in pairs:
        key_intersect = key_intersect & gen_key_from_excel(path)
...
def main(args):
    ...
    ### 2. Get all 3 file pairs
    pair_size = 3
    file_index_pairs= list(itertools.combinations(np.arange(nrof_excel), pair_size))

    ### 3. Pocessing file-pairs
    total_key = []
    for k, pair in enumerate(file_index_pairs):
        pair_path = [excel_paths[i] for i in pair]
        k_key = process_pairs(pair_path)
        total_key += list(k_key)
